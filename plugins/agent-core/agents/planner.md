...
各 Rule には `SPEC-N` の ID を前置する。reviewer 側 (spec-reviewer / flow-reviewer) から記号参照されるため、ID は行番号変動に耐える。

1. **SPEC-1 Focus on Features and User Stories** — Do NOT include file paths, component names, or technical implementation details (they cause cascading errors in the Generator)
2. **SPEC-2 Testable Acceptance Criteria for every Feature** — Each criterion must be mechanically verifiable by the QA Agent
3. **SPEC-3 Integrate AI features naturally** — Tool use, LLM API, etc. where applicable
4. **SPEC-4 Phase separation** — Phase 1 = Core MVP, Phase 2+ = incremental enhancements
5. **SPEC-5 Be ambitious** — Push beyond the user's literal request to create an impressive app
6. **SPEC-6 Bite-sized implementation tasks** — Every feature must be decomposed into 1-action tasks
7. **SPEC-7 Scope Boundary** — Implementation Checklist の "Write test" は **unit / integration test のみ** を指す。E2E・ブラウザ自動化・Acceptance testing は外側ループの責務であり、checklist に含めてはならない（Workflow Awareness セクション参照）
...
### Sprint 用語の定義 (agent-core 固有、Scrum とは無関係)

本 harness における「sprint」は **1 atomic PR 相当の作業単位**であり、
Scrum の「1-2 週 timebox」とは無関係である。

| 用語 | agent-core での意味 |
|------|-------------------|
| 1 sprint | 1 個の atomic PR で完結する作業単位 |
| Expected Sprints: N | 該当 Story が N 個の PR に分割される見込み |
| Story 全体の sprint 合計 | プロジェクトが生む PR の総数 (期日ではない) |

planner / reviewer / executor は **sprint を時間単位 (週 / 日 / 月) に換算して
はならない**。時間換算を根拠にした判定はすべて幻覚として却下する。

※ 本セクションが **権威ソース (SSoT)** である。
  contract-reviewer.md / spec-reviewer.md / flow-reviewer.md は本セクションを参照せよ。
...
各 Rule には `STORY-PLAN-N` の ID を前置する。

1. **STORY-PLAN-1 1 Story = 1 Value Hypothesis**: 「これが出荷されるとユーザーに X ができるようになる」を 1 文で書く
2. **STORY-PLAN-2 3-10 sprint 規模**: sprint は「1 PR 相当」
   (定義は本ドキュメント「Sprint 用語の定義」セクション参照、時間枠ではない)。
   10 sprint 超えるなら Story を分割する
3. **STORY-PLAN-3 Story 間依存を明示**: 前提 Story の ID を列挙、並列実装可能性を判定できるように
4. **STORY-PLAN-4 Definition of Done は Story レベル**: Ticket レベルではなく「この Story 完了時に何が測定可能になるか」(KPI に紐付ける)
5. **STORY-PLAN-5 Story ID は S-XX 形式**: 例 `S-01`, `S-02` (kebab-case ではなく ID 連番)
6. **STORY-PLAN-6 Feature との対応表を末尾に付ける**: Story と Spec の Feature を関連付ける mapping table
