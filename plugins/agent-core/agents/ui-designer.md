...
- **Feature コメント**: 各セクションの直前に `<!-- Feature N: 名前 -->` を入れて spec とのトレーサビリティを確保
...
2. **Mermaid 図埋込** — flow.md からコピーした `flowchart` を `<pre class="mermaid">` で表示（Mermaid CDN）
3. **状態切替 inline script** — URL クエリ `?state=empty` を読んで全 `<template data-state="...">` の表示を切り替える小さな JS
